...
int handle_run(connection_request_event_info *info)
{
  ...
  if (0 > base16_decode(targetpluginkey, 8, meta->obj[0].data.string.str,
    meta->obj[0].data.string.length)) {
    error_set(api_error, API_ERROR_TYPE_VALIDATION,
  ...
  hashmap_put(uint64_t, uint64_t)(callids, callid, targetpluginkey);
